...
do {
	...
	echo "---------------------------"."\n";
	//print_r(array_values(array_filter(filter_array($re)))); //调试
	$re=array_values(array_filter(filter_array($re)));

	//1为Primary，0为Secondary
	$role=!isset($re[0]['Slave_IO_State'])?1:0;
	$gtid=$re[0]['Auto_Position']==1?'ON':'OFF';
	echo "角色是:".$role_status=$role==1?'Primary'.PHP_EOL:'Secondary'.PHP_EOL;
	$is_live=isset($connect_error)?0:1;
	//echo $is_live."\n";
	$Last_IO_Error=preg_replace('/\'/', '', $re[0]['Last_IO_Error']);
	$Last_SQL_Error=preg_replace('/\'/', '', $re[0]['Last_SQL_Error']);
	...
	if($is_live==0){
		...
		//同步状态监控
	    if($re[0]['Slave_IO_Running']=='No' || $re[0]['Slave_SQL_Running']=='No' ){
			 echo "【报错】主从同步复制Slave_IO_Running状态是：{$re[0]['Slave_IO_Running']}; Slave_SQL_Running状态是：{$re[0]['Slave_SQL_Running']}\n";
	    //告警---------------------  
	    if($send_mail==0 || empty($send_mail)){
			echo "被监控主机：$ip  【{$dbname}库】【端口{$port}】关闭邮件监控报警。"."\n";
	    } else {
	    	$alarm_subject = "【报错】被监控主机：".$ip."  【{$dbname}库】【端口{$port}】主从同步复制异常 ".date("Y-m-d H:i:s");
	    	$alarm_info = "被监控主机：".$ip."   【{$dbname}库】【端口{$port}】主从同步复制Slave_IO_Running状态是：{$re[0]['Slave_IO_Running']}; Slave_SQL_Running状态是：{$re[0]['Slave_SQL_Running']}";
	    ...
	    if(!empty($recover_repl_status_row['alarm_repl_status']) && $recover_repl_status_row['alarm_repl_status'] == 1 ){
		    $recover_subject = "【恢复】被监控主机：".$ip."  【{$dbname}库】【端口{$port}】主从同步复制已恢复 ".date("Y-m-d H:i:s");
		    $recover_info = "主从同步复制Slave_IO_Running状态是：{$re[0]['Slave_IO_Running']}; Slave_SQL_Running状态是：{$re[0]['Slave_SQL_Running']}";
		    if($send_mail==1 ){
	   ...
	   //同步延迟监控
	    if(!empty($threshold_warning_repl_delay) && $re[0]['Seconds_Behind_Master']>=$threshold_warning_repl_delay){
			echo "【告警】 【{$dbname}库】【端口{$port}】主从同步延迟{$re[0]['Seconds_Behind_Master']}秒\n";
	    //告警---------------------  
	    if($send_mail==0 || empty($send_mail)){
			echo "被监控主机：$ip  【{$dbname}库】【端口{$port}】关闭邮件监控报警。"."\n";
	    } else {
	    	$alarm_subject = "【告警】被监控主机：".$ip."  【{$dbname}库】【端口{$port}】主从同步延迟 ".date("Y-m-d H:i:s");
	    	$alarm_info = "被监控主机：".$ip."   【{$dbname}库】【端口{$port}】主从同步延迟{$re[0]['Seconds_Behind_Master']}秒";
	    	$sendmail = new mail($send_mail_to_list,$alarm_subject,$alarm_info);
            $sendmail->execCommand();
	    }
	    if($send_weixin==0 || empty($send_weixin)){
	    ...
	    if(!empty($recover_repl_status_row['alarm_repl_status']) && $recover_repl_status_row['alarm_repl_status'] == 3 ){
		    ...
		    $recover_info = "主从同步延迟{$re[0]['Seconds_Behind_Master']}秒";
		    if($send_mail==1 ){
			  $sendmail = new mail($send_mail_to_list,$recover_subject,$recover_info);
			  $sendmail->execCommand();
		    }
...
//---------------------------------------------------------------------------
        ...
        if (mysqli_query($con, $sql)) {
	        ...
	        echo "---------------------------\n\n";
			mysqli_query($con,"delete from mysql_repl_status where host='{$ip}' and dbname='{$dbname}' and port='{$port}' and create_time<DATE_SUB(now(),interval 100 second)");
        } else {
